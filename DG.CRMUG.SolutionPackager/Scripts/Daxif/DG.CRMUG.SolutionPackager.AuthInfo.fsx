...
let mask key =
    ...
    let rec mask' (acc:string) (k:System.ConsoleKeyInfo) =
        match k.Key with
        | System.ConsoleKey.Enter -> cwl System.String.Empty; acc
        | System.ConsoleKey.Backspace ->
            match acc.Length with
            | 0 -> mask' acc (rk())
            | _ -> cw "\b \b"; mask' (acc.[0 .. acc.Length - 2]) (rk())
        | _ -> cw "*"; mask' (acc + string (k.KeyChar)) (rk())
...
*)
[<Literal>]
let usr = @"admin@crmugdksp00.onmicrosoft.com"
[<Literal>]
let domain = @""
[<Literal>]
let pwd = @"pass@word42"
...
[<Literal>]
let usr' = @"admin@crmugdksp01.onmicrosoft.com"
[<Literal>]
let domain' = @""
[<Literal>]
let pwd' = @"pass@word42"
